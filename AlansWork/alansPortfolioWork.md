# Portfolio Work - Software Maintenance

**Author:** Alan
**System under maintenance:** JHotDraw 7.6 (multi-module Maven, Java 8 source, 12 modules)
**Selected feature:** Automatic Selection - *Select All*, *Deselect All*, *Select Same*
**Working branch:** `AlansBranch` (feature branch off `develop`, GitHub flow)

This document collects the portfolio deliverables for the Software Maintenance course.
Each chapter corresponds to one lab. Companion files in this folder:

- [alans.md](alans.md) - step-by-step debugger / concept-location notes (Parts 1-8).
- This file - `alansPortfolioWork.md` - the portfolio chapters proper.

## Chapter index

1. **ChangeReqLab** - the change request and the three user stories US-1, US-2, US-3.
2. **CLLab** - concept location with the IDE debugger; initial 16-class table.
3. **AnalysisLab1** - packages visited during change-impact analysis (8 packages, 15 classes).
4. **RefactoringLab** - *Form Template Method* refactoring + SonarLint cleanups.
5. **ActualizationLab** - SOLID and Clean Architecture in the context of the feature.
6. **TestingLab** - JUnit 4 + Mockito unit tests (17 tests, best case + boundary + smoke).
7. **BehaviorDrivenLab** - JGiven BDD scenarios + AssertJ + AssertJ-Swing (11 scenarios).

## How to verify the work

```powershell
# Full reactor compile (12 modules)
mvn -s .maven-settings.xml --batch-mode -DskipTests verify

# All tests for the feature (TestNG + JUnit 4 + JGiven BDD)
mvn -s .maven-settings.xml --batch-mode -pl jhotdraw-actions test
mvn -s .maven-settings.xml --batch-mode -pl jhotdraw-core    test
```

Expected result at the time of writing: 12/12 modules build, 24/24 tests in `jhotdraw-actions`, 12/12 tests in `jhotdraw-core`.
...
1. Built the project (`mvn -DskipTests install`) and launched the Draw sample under the VS Code Java debugger via a project-specific [.vscode/launch.json](.vscode/launch.json) configuration pointing at `org.jhotdraw.samples.draw.Main`.
2. For each of the three sub-features (Select All, Deselect All, Select Same) I picked a *seed* class based on the menu/toolbar wiring and the `org.jhotdraw.action.edit` / `org.jhotdraw.draw.action` package names: `SelectAllAction`, `ClearSelectionAction`, `SelectSameAction`.
3. I set a breakpoint inside each seed's `actionPerformed` and triggered the matching menu item / toolbar button in the running app.
4. From each seed I followed *Step Into* through every collaborator until execution returned to the Swing event loop, recording every class encountered on the Call Stack as well as the runtime types displayed in the Variables panel.
5. Classes pulled in only as plumbing (Swing internals, `EventQueue`, etc.) were filtered out; only domain classes that *carry feature meaning* are kept in the table below.
...
## 6. SonarLint findings - additional cleanups
